...
# Define function to generate frames using pipeline
def generate_frames():
...
def pipeline(image):
    ...
    left_x_horizon = int(poly_left(horizon_y)) if poly_left is not None else 0
    right_x_horizon = int(poly_right(horizon_y)) if 'poly_right' in locals() and poly_right is not None else 0
...
def video_feed():
    with VideoFileClip("Car drive.mp4") as clip:
...
@app.route('/video_feed_withnolanedetect')
def video_feed_withnolanedetect():
    with VideoFileClip("Car drive.mp4") as clip:
        for frame in clip.iter_frames():
            try:
